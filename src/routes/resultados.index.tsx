...
function ResultsPage() {
  ...
  return <div className="space-y-4">
    ...
    <Panel title="Filtros" subtitle={`${filtered.length} ciclos localizados`} bodyClassName="grid gap-2 sm:grid-cols-2 xl:grid-cols-[1.3fr_1fr_1fr_auto]">
      ...
      <Select value={status} onValueChange={setStatus}><SelectTrigger><SelectValue placeholder="Todas as situações" /></SelectTrigger><SelectContent><SelectItem value="all">Todas as situações</SelectItem>{(["OK", "WARNING", "FAULT"] as CycleStatus[]).map((item) => <SelectItem key={item} value={item}>{cycleStatusLabel(item)}</SelectItem>)}</SelectContent></Select>
