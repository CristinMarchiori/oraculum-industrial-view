...
import { cycleStatusLabel, cycleStatusTone } from "@/lib/status";
...
function CycleDetail() {
  ...
  return <div className="space-y-4">
    ...
    <Panel title="Gráfico do ciclo" subtitle={cycle.baseFile}><ScopeLegend mode="pressure" pressureUnit={machine?.pressureUnit ?? "bar"} visible={visible} onToggle={() => undefined} /><Oscilloscope data={waveform} visible={visible} mode="pressure" pressureUnit={machine?.pressureUnit ?? "bar"} height={350} /></Panel>
    <div className="grid gap-3 xl:grid-cols-2"><PeriodTable title="Períodos sob pressão" rows={cycle.pressurePeriods} pressure pressureUnit={machine?.pressureUnit ?? "bar"} /><PeriodTable title="Alívios de pressão" rows={cycle.reliefPeriods} /></div>
...
function PeriodTable({ title, rows, pressure = false, pressureUnit = "bar" }: { title: string; rows: ProcessPeriod[]; pressure?: boolean; pressureUnit?: "bar" | "kgf/cm²" }) { return <Panel title={title} bodyClassName="p-0"><Table><TableHeader><TableRow><TableHead>Nº</TableHead><TableHead>Início</TableHead><TableHead>Fim</TableHead><TableHead>Duração</TableHead>{pressure && <TableHead>Limiar</TableHead>}</TableRow></TableHeader><TableBody>{rows.map((row) => <TableRow key={row.number}><TableCell>{row.number}</TableCell><TableCell>{row.startS.toFixed(1)} s</TableCell><TableCell>{row.endS.toFixed(1)} s</TableCell><TableCell>{row.durationS.toFixed(1)} s</TableCell>{pressure && <TableCell>{row.threshold?.toFixed(1) ?? "--"} {pressureUnit}</TableCell>}</TableRow>)}</TableBody></Table></Panel>; }